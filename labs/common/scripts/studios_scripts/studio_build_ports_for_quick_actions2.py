...
def find_deviceid_concise2(data_list, name_to_find, search_key = 'name', returned_key = 'deviceId'):
    """A one-line version using a generator expression."""
    return next((item.get(returned_key) for item in data_list if item.get(search_key) == name_to_find), None)

def find_index_concise(data_list, name_to_find, search_key='name'):
    """A one-line version to find the index of a dictionary in a list."""
    return next((index for index, item in enumerate(data_list) if item.get(search_key) == name_to_find), None)
...
## Port Profiles:
    ...
    for switchname in sorted_devices:
        structured_config_file = f'{args.avd_intended_directory}/{switchname["name"]}-debug-vars.yml'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file-interface-tsv", default='studios/studio-campus-ports.tsv', help="interface mapping for ports")
    parser.add_argument("--avd-intended-directory", default='intended/structured_configs', help="interface port profiles")
