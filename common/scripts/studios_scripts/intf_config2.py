...
def create_workspace(channel, workspace_name):
    ...
    print(f'\tWorkspace {workspace_id} creation request sent')

    # --- ADD THIS SECTION ---
    # Wait for the workspace to become active before trying to use it.
    print('\tWaiting for workspace to become active...')
    ...
    for res in stub.Subscribe(req, timeout=RPC_TIMEOUT):
        if res.value.state == workspace_models.WORKSPACE_STATE_PENDING:
